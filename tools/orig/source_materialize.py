...
def has_excluded_component(path: Path, excluded_components: tuple[str, ...]) -> bool:
...
def load_existing_source_basenames(src_root: Path, excluded_roots: tuple[Path, ...] = ()) -> dict[str, list[str]]:
    ...
    for path in sorted(src_root.rglob("*")):
        ...
        if has_excluded_component(relative_path, ("unknown", "recovered")):
            continue
...
def normalize_stub_output_relpath(path: str) -> str:
    relpath = normalize_separators(path).lstrip("/")
    if relpath.startswith(CANONICAL_ROOT_PREFIXES):
        return relpath
    if "/" not in relpath:
        return relpath
    return f"main/{relpath}"
...
def stub_output_relpath(candidate: RecoveryGroup) -> str:
    ...
    if candidate.debug_sources:
        return normalize_stub_output_relpath(candidate.debug_sources[0].path)
    return basename
...
def normalize_gap_window_output_relpath(path: str) -> str:
    return normalize_stub_output_relpath(path)
    ...
    def flush_current() -> None:
        ...
        decisions.append(
            GapWindowStubDecision(
                output_relpath=normalize_gap_window_output_relpath(str(current["path"])),
