/*
 * baby_snowworm — stationary "snow mole" baddie found in SnowHorn
 * Wastes and Dark Ice Mines. Non-aggressive. Stays buried in a snow
 * tile, periodically emerges on a counter cycle, retracts on its own
 * or when hit, and blocks path + contact-damages the player while up.
 *
 * Inferred mechanics (from the ported functions):
 *
 *   - lbl_803DE3F4 (u16)   : emerge-height counter, range 0..0xFF.
 *                            Each tick of fn_8012E050 advances it by
 *                            lbl_803DC070. Past the halfway point
 *                            (0x7F) it's fully emerged; on overflow
 *                            past 0xFF it wraps back to 0 and sets
 *                            lbl_803DC6C4 to -1 (retract-done signal).
 *   - lbl_803DE3F6 (u16)   : secondary cycle counter (wobble / anim).
 *   - lbl_803DE3FE (u8)    : "active" flag. fn_8012E0B8 resets the
 *                            two counters + error register to their
 *                            idle values whenever this flips to 0.
 *   - lbl_803DE3FF (u8)    : ramp-direction modifier for fn_8012E050
 *                            (different tick rate when set).
 *   - lbl_803DE3DB (u8)    : 3-state enum used across several
 *                            baddie DLLs (0 idle / 1 and 2 are
 *                            active phases after a hit/direction
 *                            event).
 *   - lbl_803DC6C4 (s32)   : pending-signal register, -1 when idle.
 *   - lbl_803DC70C (f32)   : a world value (volume / distance?) fed
 *                            into the frame-tick sequence.
 *
 * The 9-entry table at lbl_8031BCA0 (checked in fn_8012B9F8) is the
 * whitelist of terrain/tile types the worm can exist on. When the
 * tile underneath the worm is not one of those 9, fn_8012B9F8 returns
 * 0 and the caller presumably aborts emergence — this is the "ice
 * melts → worm can't pop up" mechanic.
 *
 * Owner: main/dll/baddie/baby_snowworm.c
 * Text span: 0x80128260-0x8012EBBC  (32 functions, ~27KB)
 *
 * Matching (100% retail bytes): 7 / 32
 *   fn_8012E0E0 : read lbl_803DE3D9        (idle/variant byte getter)
 *   fn_8012E0E8 : read low byte of lbl_803DE3F6
 *                                          (wobble-byte getter)
 *   fn_8012E0B8 : set active flag; if idle, zero both counters and
 *                 set lbl_803DC6C4 = -1    (reset to buried state)
 *   fn_8012E050 : advance emerge-height by lbl_803DC070, gated by
 *                 lbl_803DE3FF and the "above-halfway" check; on
 *                 overflow past 0xFF wraps to 0 and raises -1 signal
 *   fn_80129FB0 : frame-update sequence — 6 external calls with a
 *                 shared float value from sdata
 *   fn_8012E250 : "emerge" dispatch — set enum = 1, vtable call
 *                 (anim/sound id 0x94), raise two HUD/AI flags
 *   fn_8012B9F8 : terrain whitelist check — compare tile byte against
 *                 the 9-entry lbl_8031BCA0 table
 *
 * The remaining 25 are stubs from the Ghidra bulk import. Several
 * more were attempted and reverted because MWCC -O4,p's CSE /
 * scheduler / peephole picks a different shape than retail:
 *
 *   fn_8012E0F4  : 32b "if (x) store=1 else store=0". MWCC always
 *                  emits branchless cntlzw/srwi/addi; retail has an
 *                  explicit cmplwi/beq/li 1/b/li 0 dispatch. Neither
 *                  #pragma scheduling off nor #pragma peephole off
 *                  suppresses the branchless transform.
 *
 *   fn_8012E114  : 316b state setter with a four-bit flags arg.
 *                  Retail emits `stb r28; clrlwi r0, r28, 24` (store
 *                  then compute index); MWCC CSE's the `(u8)arg1`
 *                  and emits `clrlwi r0, r28, 24; stb r0` (compute
 *                  once, reuse). The register alloc that follows
 *                  then diverges in the table-index path.
 *
 *   fn_8012C1C0  : 380b frame-tick handler. Retail packs `char +
 *                  char[15]` tight into a 0x20 stack frame (scratch
 *                  starts at r1+0x9, right after the byte at r1+0x8).
 *                  MWCC 4-byte-aligns the 15-byte array, giving a
 *                  0x30 frame. No pragma controls this padding.
 *
 *   fn_801299D4  : 196b state mutator. Retail's final store of the
 *                  byte arg is a plain `stb r28, lbl(r13)`; MWCC
 *                  always emits `extsb r0, r28; stb r0, lbl(r13)`,
 *                  irrespective of whether the arg / lbl / both are
 *                  typed char, u8, s8, or any combination of casts.
 *                  Looks like MWCC's type-narrowing path always goes
 *                  through extsb for small-int global stores in this
 *                  version.
 *
 *   small 1-call wrappers : `li constN` for the second arg is
 *                  scheduled ahead of the prologue's `stw r0, 0x14`
 *                  for latency hiding. '#pragma scheduling off'
 *                  fixes this class (see fn_802BBAFC / fn_802BB754
 *                  in TRK stubs for the proven pattern).
 */
 ...
 * EN v1.0 Size: 72b
 *
 * Frame-update sequence. Begins the frame (fn_8000F478(0)) and, if a
 * secondary subsystem is live (lbl_803DE460 != 0), runs its tick via
 * fn_8000FAF8. Then pushes a float from lbl_803DC70C (probably a
 * distance or alpha scalar for the next render pass) through the
 * standard baddie rendering chain: ...F584 -> ...FC5C(f) -> ...FB20
 * -> ...F7A0. These five calls appear in other baddie DLLs' frame
 * funcs in the same order, suggesting a shared baddie-frame ABI.
 */
 ...
 * EN v1.0 Size: 192b
 *
 * Terrain/tile whitelist check. Pulls the current game object from
 * fn_8002BAC4; aborts if it's NULL or if fn_80296328() reports no
 * relevant state. Then fetches a tile-type byte either directly from
 * the object's 0xAC-offset sub-struct, or — if that sub-struct is
 * absent — by sampling world-space via fn_8005B128(xf, yf), where xf
 * and yf are the floats at offsets 0xC and 0x14 of the object
 * (its XY position). The byte is scanned against the 9-entry
 * lbl_8031BCA0 table; a match returns 0 (abort emerge), otherwise 1.
 * This is the "ice melted → can't pop up" / "not on snow → hide"
 * gatekeeper.
 */
#pragma scheduling off
#pragma peephole off
 ...
 * EN v1.0 Size: 104b
 *
 * Emerge-height tick. Does nothing while lbl_803DE3F4 == 0 (buried /
 * not emerging). Once in motion:
 *   - if lbl_803DE3FF != 0 (faster/slower ramp mode) and height is
 *     still under 0x7F (first half of the emerge arc) → add
 *     lbl_803DC070 to height.
 *   - otherwise if lbl_803DE3FF == 0 → always add lbl_803DC070
 *     (normal ramp).
 *   - if height > 0xFF after the add → fully cycled: reset to 0 and
 *     write -1 to lbl_803DC6C4 (retract-done signal for the outer
 *     FSM to re-arm / pick the next emerge).
 * Called once per game frame while the worm is mid-cycle.
 */
#pragma scheduling off
void fn_8012E050(void)
{
 ...
 * EN v1.0 Size: 40b
 *
 * Set the "active" flag to param_1 and — if that just put the worm
 * into idle (param_1 == 0) — zero both cycle counters
 * (lbl_803DE3F4, lbl_803DE3F6) and set the pending-signal register
 * lbl_803DC6C4 to -1. Passing a nonzero value just flips the flag
 * without touching the counters, so live emerges aren't interrupted.
 */
#pragma scheduling off
#pragma peephole off
void fn_8012E0B8(u8 param_1)
 ...
 * EN v1.0 Size: 8b
 *
 * Idle/variant byte getter. Returns the sbss byte at lbl_803DE3D9,
 * which appears to be a small enum set at spawn and never changed by
 * this DLL (likely the instance's appearance variant or spawn slot).
 */
u8 fn_8012E0E0(void)
{
  return lbl_803DE3D9;
 ...
 * EN v1.0 Size: 12b
 *
 * Wobble-byte getter. Returns the low 8 bits of the u16 wobble
 * counter at lbl_803DE3F6. The counter is a halfword (the full
 * 16-bit value is read and written elsewhere), but this accessor
 * deliberately narrows to a byte — most likely the wobble is used
 * as a u8 anim-frame index by its callers.
 */
u8 fn_8012E0E8(void)
{
 ...
 * EN v1.0 Size: 84b
 *
 * "Emerge" dispatch. Sets the hurt-state enum lbl_803DE3DB = 1
 * (active phase), then fires a vtable call at offset 0x24 of the
 * baddie-system object at *lbl_803DD6D0 with (1, 0x94, 1). 0x94 is
 * very likely the emerge animation or SFX id. Follows up with
 * fn_800207AC(1) (raise AI-priority or HUD marker) and
 * fn_800206EC(0xff) (set draw alpha / visibility to full).
 */
#pragma scheduling off
void fn_8012E250(void)
{
